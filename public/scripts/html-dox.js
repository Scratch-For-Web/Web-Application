...
if(!imports) {
  console.error("No imports specified");
}
let debugOn = document.querySelector("meta[debug]")
  ? document.querySelector("meta[debug]").getAttribute("debug")
  : false;
let hooked = false;
...
let methods = {
  ...
  title: (title) => {
    if (hooked) {
      throw new Error(
        "Cannot send headers after they where already sent please refrain from using double res functions and call res.return() to resend to client"
      );
    }
  ...
  sendStatus: (msg, code) => {
    ...
    if (typeof code === "number") {
      document.getElementById(methods.rootElement).innerHTML = JSON.stringify({
        msg,
        code,
      });
      hooked = true;
      ...
      if (!this.storedroutes.includes(window.location.hash.substring(1))) {
        ...
        if (this.customerror === null) {
          document.getElementById(
            this.rootElement
          ).innerHTML = `<code>Cannot GET ${window.location.hash.substring(
            1
          )}</code>`;
        } else {
          callback(res);
  ...
  root(path, callback) {
    ...
    if (window.location.hash.substring(1).match(regex)) {
      ...
      if (
        ...
        console.error(`
               [DoxDom: Cannot use query params with path params ${path} ${
          window.location.hash.substring(1).split("?")[1]
        }
              `);
...
function setDox(html) {
  let dox = {
    querySelector: function (selector) {
      let el =
        document.querySelector(selector) || html.body.querySelector(selector);
    ...
    querySelectorAll: function (selector) {
      let el =
        ...
        html.body.querySelectorAll(selector);
      let els = [];
    ...
    getId: function (id) {
      let el =
        html.body.querySelector("#" + id) || document.querySelector("#" + id);
      return doxMethods(el);
    },
    ...
    validate: function (type, value, options) {
      if (type === "email") {
        ...
        let time =
          options.await && options.await.time ? options.await.time : null;
...
function handleForms(html) {
  ...
  forms.forEach(async function (form) {
    ...
    if (submitId) {
      dox.awaitElement("#" + submitId).then(function (element) {
        document
          .querySelector("#" + submitId)
          .addEventListener("click", function (e) {
            let data = {};
            let inputs = [
              ...document.querySelectorAll(
                'form[action="' + form.getAttribute("action") + '"] input'
              ),
            ];
            inputs.forEach(function (input) {
              if(input.pattern && !input.value.match(input.pattern)) {
                action = null;
                return;
              }else{
            ...
            if (action) {
              ...
              router.post(action, function (res) {
...
function handleLogic(html) {
  ...
  while (match) {
    ...
    if (executeJs(condition)) {
      processedStatement = ifStatement.includes("return")
        ? ifStatement
        : `<script>${ifStatement}</script>`;
    } else {
      processedStatement =
        elseStatement && !elseStatement.includes("#if")
          ? elseStatement.includes("return")
            ? elseStatement
...
function handleState(html) {
  ...
  if (matches) {
    matches.forEach(function (m) {
      ...
      if (
        html.body.querySelector("dox-shadow") &&
        html.body.querySelector("dox-shadow").innerHTML.includes(m)
      ) {
        if (debugOn) {
          ...
          slot.querySelectorAll("*").forEach(function (element) {
            ...
            for (let i = 0; i < attributes.length; i++) {
              if (attributes[i].value.includes(m)) {
                attributes[i].value = attributes[i].value.replaceAll(
                  m,
                  stateValue
                );
                memory[stateName] = {
    ...
    for (let stateName in memory) {
      effect(stateName, async function (value) {
        ...
        el.innerHTML = el.innerHTML.replaceAll(
          memory[stateName].currentValue,
          value
        );

        memory[stateName].currentValue = value;
        window.postMessage(
          { dox: "stateChange", message: " Do not update", kill: "$true" },
          "*"
        );
...
function handleMarkdown(html) {
  ...
  code.forEach(function (c) {
    ...
    h = h.replace(
      linkRegex,
      '<a  style="color: #007bff; text-decoration:none" href="$2">$1</a>'
    );
    h = h.replace(
      quoteRegex,
      '<blockquote style="border-left: 5px solid #eeeeee; padding-left: 10px;">$1</blockquote>'
    );
...
function handleScripts(html) {
  if (html) {
    ...
    for (var i = 0; i < scripts.length; i++) {
      ...
      if(!script.hasAttribute("key") && !script.hasAttribute("data-type") && !script.hasAttribute("src")){
        throw new Error("Scripts must have a key for variables and props to work")
      }
      let key = script.getAttribute("key") || ""
      if (
        !script.hasAttribute("execute") &&
        !script.hasAttribute("props") &&
        !script.hasAttribute("defered") &&
        ...
        while ((match = variableRegex.exec(s)) !== null) {
          ...
          if(!window.variables[key]){
            window.variables[key] = {}
          }

          window.variables[key][name] = value;
        ...
        var s = script.innerHTML;
        if (!script.hasAttribute("key") && debugOn) {
          throw new Error(
            "Props must have a key attribute - to prevent duplicate props"
          );
        }
        let key = script.getAttribute("key");
        var match;
        ...
        while ((match = variableRegex.exec(s)) !== null) {
          ...
          // use key to store prop to fix duplicate props
          window.props[key + "." + name] = value;
          if (!window.props[key]) {
            window.props[key] = {};
          }
          window.props[key][name] = value;
    ...
    let fetchPromise = fetch(src)
      ...
      .then(function (data) {
        ...
        return handleImports(d).then(function (updatedHtml) {
          ...
          if (
            htm.querySelector(name) &&
            htm.querySelector(name).querySelector("slot")
          ) {
          ...
          if (matches) {
            matches.forEach(function (match) {
              html.body.outerHTML = html.body.outerHTML.replaceAll(
                match,
                "<!-- " + match + " -->"
              );
            });
...
function processElement(el) {
  if (debugOn) {
    console.log(
      "[Processing Engine]: Deriving values from dynamically appended element -> ",
      el.element
    );
  }
  el = el.element;
  ...
  importedElements.forEach(function (importedElement) {
    if (importedElement.name === tagName) {
      ...
      for (let i = 0; i < el.attributes.length; i++) {
        ...
        if (debugOn) {
          console.log(
            `[Processing Engine]: Setting attribute for <${tagName}/> -> `,
            name,
            value
          );
        }
      ...
      document.querySelectorAll(tagName).forEach(function (e) {
        ...
        try {
          ...
          if (debugOn) {
            console.log(
              "[Processing Engine]: Error replacing element -> ",
              error
            );
          }
        }
        if (debugOn) {
          console.log(
            "[Processing Engine]: Replacing dynamic element -> ",
            e,
            duplicate
          );
        }
        ...
        addedNodes.forEach(function (node) {
          if (node.tagName) {
            ...
            let el =
              ...
              document.body.querySelectorAll(tagName);
            let parent = node.parentNode;

            if (debugOn) {
              console.log(
                "[Processing Engine]: New element added -> ",
                el,
                parent
              );
            }
            if (!window.renderLock) {
              ...
              if (document.body.innerHTML.includes("${{")) {
                let matches =
                  document.body.innerHTML.match(/\${{([\s\S]*?)}}/gs);
                if (matches) {
                  matches.forEach(function (match) {
                    ...
                    try {
                      ...
                      if (debugOn) {
                        console.error(
                          "%c[Js Parse Engine]: 😭 Error parsing " + match,
                          "reason: " + error,
                          "parent",
                          el
                        );
                    ...
                    document.body.innerHTML =
                      document.body.innerHTML.replaceAll(match, value);
                    if (debugOn) {
                      // dark green text
                      console.log(
                        `%c[Js Parse Engine]: Replacing ${match} with -> `,
                        "color: #006400",
                        value
                      );
                    }
  ...
  .then(function (fetchedTemplates) {
    ...
    return Promise.all(
      templates.map(function (template) {
        ...
        if (matches) {
          matches.forEach(function (match) {
            d.body.outerHTML = d.body.outerHTML.replaceAll(
              match,
              "<!-- " + match + " -->"
            );
            if (debugOn) {
              console.log(
                "[Comment Parser]: Replacing" +
                  "```" +
                  match +
                  "```" +
                  " with " +
        ...
        beforeRenderScripts.forEach(function (script) {
          if (script.includes("fetch")) {
            ...
            if (!document.getElementById(id)) {
              ...
              try {
                ...
                if (debugOn) {
                  console.log(
                    "[BeforeRender Execution Engine]: something went wrong -> ",
                    error
                  );
                }
              }
          ...
          if (debugOn) {
            ...
            let msg =
              "[BeforeRender Execution Engine]: Executing script -> " + script;
      ...
      if (template.template.data.includes("${{")) {
        ...
        if (matches) {
          matches.forEach(function (match) {
            ...
            template.template.data = template.template.data.replaceAll(
              match,
              value
            );
          });
      ...
      if (importElement.endsWith(".js")) {
        fetch(importElement)
          ...
          .then((data) => {
            ...
            let id =
              importElement.split("/").pop().split(".")[0].toLowerCase() +
              "-script";
