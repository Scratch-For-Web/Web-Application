...
<script>
	...
	import {
		WorkspaceSearch
	} from "@blockly/plugin-workspace-search";
			...
			function prepToolbox(toolbox_content, searching, favorites, pooopewwweewwww, searchparameter) {
				...
				for (var count = 0; count < repeat_end; count++) {
					if (toolboxArray[count].includes('<block type="') && !toolboxArray[count].includes("LINE HIDDEN FROM SEARCH")) {
						pushed = toolboxArray[count].replace(/ /g, "").replace(/blocktype="/g, "").replace(/\//g, "").replace(/</g, "").replace(/"/g, "").replace(/'/g, "").replace(/\t/, "");
            ...
            '"/>\n<block type="'
					...
					if (newblocks.length > 0) {
						...
						if (length_lessthan_350 == false) {
							CATEGORYCONTENT = CATEGORYCONTENT + `<label text="${
				...
				var returned_stuff = toolbox_content.replace("<!-- CATEGORY_CONTENT_VARIABLE_GOES_HERE_897489712470376894703168263487623 -->", `<label text="There are currently ${String(
          count_of_blocks
			...
			async function reloadWorkspace2(workspace, abc) {
				...
				/*Blockly.ContextMenuRegistry.registry.register({
				  ...
				  callback: async function (scope) {
				    ...
				    if (
				      arrayRemove(await localforage.getItem("p"), type).length === 0
				    ) {
...
*/
			...
			function workspaceToSvg_(workspace, callback, customCss) {
				...
				var css = [].slice.call(document.head.querySelectorAll("style")).filter(function(el) {
...
*/
					...
					for (let i = 0; i < elem2ents.length; i++) {
						...
						for (let i = 0; i < elements.length; i++) {
							...
							if (current.getAttribute("d").match(eventBlockSVGRegex) != null) {
								const width = Number(current.getAttribute("d").match(/h [0-9\.]*/im)[0].replace("h ", ""));
								current.setAttribute("d", `m 0 0 c 25 -22 71 -22 96 0 H ${
                  width + 3.5
					...
					for (let i = 0; i < elem2ents.length; i++) {
						...
						for (let i = 0; i < elements.length; i++) {
							...
							if (current.getAttribute("d").match(outputBlockSVGRegex) != null) {
								let width = Number(current.getAttribute("d").match(/h [0-9\.]*/im)[0].replace("h ", "")) - 55;
			...
			if (String(window.location.pathname).replace(/\//gmi, "") == "funni") {
				...
				element.loop = true
				element.src = origin + "/funni/audio/stringsbreakfree.mp3"
				var myImg = document.createElement('img');
  myImg.alt = 'annoying image';
  myImg.src = origin + '/funni/dance.gif';
  myImg.height = 200;
...
Toast.fire({
  ...
  title: 'Please press anywhere'
        ...
        return Math.floor(Math.random() * (b - a + 1) + a);
				...
				while (true) {
        await delay(300);
					div.style.top = `${mathRandomInt(1, 100)}vh`;
    }
				})();
			};
		},
